...
import { X, Copy, Check, Sparkles, Terminal, History, Plus, Trash2, Pencil, Save, AlertCircle, Wand2, Loader2 } from 'lucide-react';
...
export const PrismModal: React.FC<PrismModalProps> = ({ prompt, isOpen, onClose, onUpdatePrompt, onDeletePrompt }) => {
    ...
    const handleAiGenerate = async () => {
        ...
        try {
            const newContent = await api.generatePrompt(aiInstruction, editContent, apiKey, model);
                                    ...
                                    <div className="mb-4">
                                        {isAiOpen ? (
                                            <motion.div
                                                ...
                                                className="bg-purple-500/10 border border-purple-500/20 rounded-lg p-3 flex gap-2"
                                            >
                                                <input
                                                    autoFocus
                                                    value={aiInstruction}
                                                    ...
                                                    placeholder="Ask AI to improve or generate..."
                                                    className="flex-1 bg-transparent text-sm text-white placeholder-purple-300/50 focus:outline-none font-mono"
                                                    disabled={isGenerating}
                                                />
                                                <button
                                                    onClick={handleAiGenerate}
                                                    disabled={isGenerating}
                                                    className="bg-purple-500 hover:bg-purple-400 text-white p-1.5 rounded transition-colors disabled:opacity-50"
                                                >
                                                    {isGenerating ? <Loader2 className="w-4 h-4 animate-spin" /> : <Wand2 className="w-4 h-4" />}
                                                </button>
                                                <button onClick={() => setIsAiOpen(false)} className="text-purple-300 hover:text-white p-1.5">
                                                    <X className="w-4 h-4" />
                                                </button>
                                            </motion.div>
                                        ) : (
                                            <button
                                                ...
                                                className="text-xs font-mono text-purple-400 hover:text-purple-300 flex items-center gap-2 transition-colors"
                                            >
